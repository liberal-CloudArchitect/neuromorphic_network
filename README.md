...
当前必须先在 `p4-protocol-v2` 的新 clean SHA 上重建 CI、qualification 与 pilot lock，再从新 run ID 启动 48 小时 mechanism；不得恢复或拼接两个 v1 资源停止 run。资格和 pilot 不构成预测有效、稀疏路由非劣、泛化改善或 network MVP 结论；版本继续保持 `0.4.0`。
