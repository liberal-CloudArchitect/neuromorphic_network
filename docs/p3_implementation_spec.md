...
- 正式 run `p3-full-dc6c259c-20260719T034230Z` 在 MPS 上完成 81/81 cells，实际 suite 墙钟 49.16 小时。
- `GATE-3 PASSED`：矩阵、三 seed、checksum、逐样本证据、恢复与统计完整。
- `GATE-NN-MVP FAILED`：predictive causal 未通过；AULC 未同时胜过 GRU 与 Transformer；稀疏执行超过 2pp 非劣界。
- `DR-001`～`DR-003` 记录 formal metric 与容量字段缺口。它们只能阻止相关类别通过，不能用于事后修改阈值或反转 Gate。
- 项目可作为 `0.4.0` 科研工具继续迭代，但当前不生成或发布 `network-mvp-v1` 正式 bundle。
