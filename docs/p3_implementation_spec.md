...
- 正式 MPS 矩阵总墙钟最多 72 小时。超限只在 checkpoint 边界停止并记录 `RESOURCE_LIMIT`，不得删减矩阵后宣称完成。
...
- 顺序学习按 seed 使用 Latin orders：17=`AR→DRS→SG`、29=`DRS→SG→AR`、43=`SG→AR→DRS`，每阶段 1,500 updates。`forgetting = 历史最佳 analysis score - 最终 score`。
...
- `qualification` profile 使用 seed 7、`64/32/32/32/32` 样本和缩小 updates，覆盖所有 cell 类型、恢复、干预、统计和产物，但必须标记 `qualification_only=true`。
