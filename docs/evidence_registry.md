...
- `ENG-P5-RESUME-001`：checkpoint-v6 在任何 live mutation 前验证 schema、profile/candidate cursor、config/protocol hash、model/optimizer groups、RNG、NetworkState 与严格递增 analysis curve；CPU roundtrip 逐位恢复并拒绝错误 hash。
...
- `QUAL-P5-MECH-CPU-001`：历史 v1 dirty-worktree CPU micro 完成 8/8 cells；其 checkpoint-v5 与锁已被 `CR-012` 退役，不得授权 protocol-v2 正式运行。
- `ENG-P5-VALIDITY-002`：protocol-v2 对 fixed-budget analysis AULC、validation/analysis 隔离、best checkpoint 恢复、summary 独立复算、诊断损失零权重及无 host-scalar-sync 路由建立反假阳性测试。
